...
## Does the GHz radio excess (think ARCADE2) come from z<0.6 structure?##
Holder 2011 (?) argues pretty convincingly that this cannot be the case.  If it comes from LSS, it must cluster, and the the GHz sky is too smooth.  However, as Gil points out, there are ways out, like it could be very puffy (10 Mpc radio halos or something), and would thereby evade the "overly smooth" ell~3000 anisotropy data that Gil used.

The idea here is to test this on degree scales, by cross-correlating BOSS galaxy survey data with ~GHz sky data.
...
7. Consider using several ~GHz radio surveys.
